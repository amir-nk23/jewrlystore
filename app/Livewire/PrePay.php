...
//        $profit=$list[0]->delay;
//        $price=$list[0]->price;
//        $date=Carbon::create(2023,11,4);
//        $diff=$date->diffInDays($end_date);
//        $per_P=$price/3;
//        $this->pre_p = number_format($per_P+($diff*$profit*$price),0,',');
//
//    }
...
//        $list = Installment::all();
//
//        $payment = $list[0]->price;
//        $month=$list[0]->month;
//        $profit=$list[0]->profit;
        $id=session('id');

        $list=Notebook::where('installment_id',$id)->get();
        $list->load([
           'installment'
        ]);

        return view('livewire.pre-pay',compact('list',));
    }
